...
def prettylist(lst, remove_br=False):
    ...
    Description: This function concatenates a list of strings in order to be viewed nicely when using Folium. It ensures that
    the field does not run off the page when viewing points on an interactive Folium map.
    '''
    people_string = ''
    char_count = 0
    index = 0
...
def find_ppg_data(ppg_df, country):
    '''
    Parameter(s): DataFrame (People groups data frame. MUST be cleaned according to other functions)
    Parameter(s): String (Filepath to shp_file containing subdivision spatial data)
    Parameter(s): String (Name of the administrative level 1 subdivision name - e.g. Province)
    Descriptions: Finds all the
    ...
    People Groups that have a country of origin and Deaf peoples. This function also reformats the language variable.
    '''
...
def convert_to_geodataframe(df):
    ...
    Output: GeoDataFrame
    Description: This function converts the dataframe to a geodataframe by using the Latitude and Longitude columns 
    given from the PeopleGroups spreadsheet.
    Note: This method should eventually be replaced by a pipeline function that does not already require a cleaned PeopleGroups dataset.
    """
    ...
    df = df[['IMB_Affinity', 'Country', 'People_Group', 'People_Name', 'Population', 'Language', 'Religion', 'Latitude', 'Longitude']]
    df_copy = df.copy()
