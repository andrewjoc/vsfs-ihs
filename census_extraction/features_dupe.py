...
def find_all_adm(ppg_gdf, pop_data, verbose):
    '''
    input: ppg_gdf (geodataframe), pop_data (dataframe), verbose (boolean)
    output: people_areas (dataframe)
    description: Helper function for validate_country. Finds all the ADM boundaries that a people group intersects.
    '''
    ...
    for people_polygon in ppl_areas.geometry:
        ...
        all_boundaries_found = pop_data.query('index in @indices')['ADM3 Name'].to_list()
    ...
    ppl_areas['ADM3 Boundaries Present'] = boundaries     
    
    # add the total boundary population column
    ppl_areas['Total Boundary Population'] = ppl_areas['ADM3 Boundaries Present'].apply(lambda x: find_total_boundary_pop(x, pop_data))
    ...
    if verbose:
        ...
        for ppl_group in invalid_people_groups['People Group']:
            if invalid_people_groups[invalid_people_groups['People Group'] == ppl_group]['ADM3 Boundaries Present'].iloc[0] == 'NONE':
                print(f'The {ppl_group} people group did not intersect with an ADM3 boundary. They may be valid.')
                ...
                print(f'The {ppl_group} people group had a population greater than all ADM3 boundaries they intersected. They are invalid.')            
...
def find_all_adm2(ppg_gdf, pop_data, verbose):
    '''
    input: ppg_gdf (geodataframe), pop_data (dataframe), verbose (boolean)
    output: people_areas (dataframe)
    description: Helper function for validate_country. Finds all the ADM boundaries that a people group intersects.
    ...
    for people_polygon in ppl_areas.geometry:
        ...
        # select the names of the boundaries
        all_boundaries_found = pop_data.query('index in @indices')['ADM2 Name'].to_list()
    ...
    # create new series that is the list of all boundaries present
    ppl_areas['ADM2 Boundaries Present'] = boundaries     
    
    # add the total boundary population column
    ppl_areas['Total Boundary Population'] = ppl_areas['ADM2 Boundaries Present'].apply(lambda x: find_total_boundary_pop_adm2(x, pop_data))
    ...
    if verbose:
        ...
        for ppl_group in invalid_people_groups['People Group']:
            if invalid_people_groups[invalid_people_groups['People Group'] == ppl_group]['ADM2 Boundaries Present'].iloc[0] == 'NONE':
                print(f'The {ppl_group} people group did not intersect with an ADM2 boundary. They may be valid.')
            elif invalid_people_groups[invalid_people_groups['People Group'] == ppl_group]['People Group Population'].iloc[0] >= invalid_people_groups[invalid_people_groups['People Group'] == ppl_group]['Total Boundary Population'].iloc[0] * 1.05: # error of 5%
                print(f'The {ppl_group} people group had a population greater than all ADM2 boundaries they intersected. They are invalid.')            
...
def find_total_boundary_pop(lst, subnational_data):
    ...
    pop_sum = 0
    for item in lst:
        pop_sum += subnational_data[subnational_data['ADM3 Name'] == item]['ADM3 Population 2020'].iloc[0]
    return pop_sum


def find_total_boundary_pop_adm2(lst, subnational_data):
    '''
    input: lst (list), subnational_data (dataframe)
    output: pop_sum (float)
    description: Helper function for find_all_adm1
    ...
    pop_sum = 0
    for item in lst:
        pop_sum += subnational_data[subnational_data['ADM2 Name'] == item]['ADM2 Population 2020'].iloc[0]
    return pop_sum
...
def validate_png(countrydf, verbose=True):
    '''
    input: country (str), verbose (boolean)
    output: dataframe
    description: Returns a Pandas dataframe validating all the people groups within a certain country. If verbose=True, it prints out current status of the function and why some groups did not pass the validation
    ...
    return find_all_adm(country_ppg, countrydf, verbose)


def validate_ind(countrydf, verbose=True):
    '''
    input: country (str), verbose (boolean)
    output: dataframe
    description: Returns a Pandas dataframe validating all the people groups within a certain country. If verbose=True, it prints out current status of the function and why some groups did not pass the validation
